...
int main(int argc, char** argv)
{
  ...
  float alpha;
  float beta;
  float (*tmp)[ni][nj]; tmp = (float(*)[ni][nj])malloc ((ni) * (nj) * sizeof(float));
  float (*A)[ni][nk]; A = (float(*)[ni][nk])malloc ((ni) * (nk) * sizeof(float));
  float (*B)[nk][nj]; B = (float(*)[nk][nj])malloc ((nk) * (nj) * sizeof(float));
  float (*C)[nj][nl]; C = (float(*)[nj][nl])malloc ((nj) * (nl) * sizeof(float));
  float (*D)[ni][nl]; D = (float(*)[ni][nl])malloc ((ni) * (nl) * sizeof(float));
